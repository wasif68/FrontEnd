...
import { applyToJob, saveJob, unsaveJob, shareJob } from "@/services/jobService";
import { copyToClipboard } from "@/utils/clipboard";
import Toast from "@/components/Toast";
import "./RecommendationsTab.css";

export default function RecommendationsTab({ profile: externalProfile, onProfileUpdate }) {
  ...
  useEffect(() => {
    ...
    const loadProfile = async () => {
      if (currentUser && currentUser.id) {
        try {
          // Try backend API
          const response = await fetch(`http://localhost:3100/api/user/${currentUser.id}/profile`);
          if (response.ok) {
            ...
            setProfile({
              ...
              skills: JSON.parse(userData.skills_selected || "[]"),
            });
            return;
          }
        } catch (error) {
          console.warn("Error loading profile from API:", error);
        }
      }
  ...
  const handleSave = useCallback(async (jobId, shouldSave) => {
    if (!currentUser?.id) {
  ...
  }, [currentUser?.id, updateSavedJobs, refreshUserData]);

  // Handle Share button
  const handleShare = useCallback(async (jobId) => {
    if (!currentUser?.id) {
      ...
      <div className="jobs-grid">
        {scored.length === 0 ? (
          ...
          scored.map((job) => {
            ...
            return (
            <div key={job.id} className={`job-card transition-all duration-150 ease-in-out transform hover:scale-102 active:scale-95 active:shadow-md ${isSaved ? 'saved' : ''}`}>
              {/* Job Header */}
                  ...
                  <div className="job-company">
                    ...
                    {job.companyRating && (
                      <div className="company-rating">
                        ...
                        <span className="reviews-count">({job.companyReviews})</span>
                      </div>
                    )}
                  </div>
                ...
                <div className="match-reason-container">
                  ...
                  >
                    <svg 
                      ...
                      fill="none" 
                      stroke="currentColor"
                      className={`match-reason-icon ${expandedReasons.has(job.id) ? 'expanded' : ''}`}
                    >
                      <polyline points="6 9 12 15 18 9"/>
                    </svg>
                    <span>Why this matches ({job.matchedSkills.length} skills)</span>
                  </button>
                  {expandedReasons.has(job.id) && (
                    <div className="match-reason-content">
                      ...
                      <span>Matched skills: <strong>{job.matchedSkills.join(", ")}</strong></span>
                    </div>
                  )}
                </div>
              )}
